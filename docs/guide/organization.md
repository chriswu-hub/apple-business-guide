...
1. **步驟 1：向經銷商索取「經銷商編號」**
   - **白話解釋**：向合作的 Apple 授權經銷商索取其在 Apple 登記的經銷商編號（或 Apple 客戶編號 / Customer Number）。
   - **目的**：經銷商會提供一組專屬官方代碼，代表其官方經銷商身分。

2. **步驟 2：在 Apple 商務後台「加入經銷商編號」**
   - **白話解釋**：告訴 Apple 官方：*「我信任這家經銷商，未來只要是這家經銷商出貨給我的設備，我都允許它登錄進我的公司後台。」*
   - **操作路徑**：登入 `business.apple.com` > 點擊頂部導覽列**「裝置」** > 點擊左側側邊欄**「庫存」** > 點選右上角的 **「+」**（新增 Apple 客戶編號 / 經銷商編號）：
   <div class="step-image-container">
     <img src="/images/reseller_step1_inventory.png" alt="裝置 > 庫存 > 新增編號" class="step-image" />
   </div>
   點選 **「+」** 後，在彈出視窗中輸入經銷商提供的詳細資料並確認（以請經銷商提供經銷商編號為例）：
   <div class="step-image-container">
     <img src="/images/reseller_step2_add_details.png" alt="輸入經銷商詳細資料" class="step-image" />
   </div>

3. **步驟 3：提供公司的「機構 ID (Organization ID)」給經銷商**
   - **白話解釋**：這是**雙向確認**。在 Apple 商務後台取得企業的身分證字號（機構 ID）。
   - **操作路徑**：將機構 ID 提供給經銷商業務綁入其進銷存系統，出貨時系統才知道要把這批設備序號派送到哪一家組織。
...
```
[企業組織] ──(1. 索取經銷商編號)──> [授權經銷商]
[企業組織] ──(2. 後台登記經銷商編號)──> [Apple 雲端認證]
[企業組織] ──(3. 提供機構 ID)────────> [經銷商系統綁定]
                  ⬇
